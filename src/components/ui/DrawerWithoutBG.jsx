...
export const DrawerWithoutBG = ({children, title, icon, show, setShow}) => {
	return (
		<>
			<div
				className={`top-0 left-0 z-[60] fixed w-svw h-svh transition-all
				${show ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
				<div
					className={`rounded-tr-[34px] rounded-tl-[34px] bg-white h-[calc(100svh-30px)] right-0 fixed w-full shadow pt-9 px-8 lg:px-10 transition-all delay-150
						dark:bg-secondary-800 custom-dark:bg-secondary-800
						lg:top-0  lg:max-w-[440px] lg:h-svh lg:rounded-none
					...
					<h2 className="flex gap-4 font-medium text-[22px] lg:text-xl mb-4 capitalize dark:text-white custom-dark:text-white">
						{icon ? <Icons name={icon}/> : null} {title}
					</h2>
